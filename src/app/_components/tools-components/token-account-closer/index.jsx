...
const VaporToolScreen = () => {
    ...
    try {
      ...
      const refund =
        processedAccounts
          ...
          .reduce((total, account) => total + account.rentExemptLamports, 0) /
        LAMPORTS_PER_SOL;
      ...
      toast.error(
        "Failed to fetch token accounts: " + (err.message || "Unknown error")
      );
    } finally {
  ...
  const closeSelectedAccounts = async () => {
    const selectedKeys = Object.keys(selectedAccounts).filter(
    ...
    try {
      ...
      const closedAccounts = tokenAccounts.filter(
        (acc) => selectedAccounts[acc.pubkey]
      );
      const refundAmount =
        closedAccounts.reduce(
          (total, acc) => total + acc.rentExemptLamports,
          0
        ) / LAMPORTS_PER_SOL;
      ...
      toast.success(
        ...
        } account(s) and recovered approximately ${refundAmount.toFixed(6)} SOL`
      );
      ...
      toast.error(
        "Failed to close accounts: " + (err.message || "Unknown error")
      );
    } finally {
      setLoading(false);
    }
        ...
        <div className="flex items-center justify-center min-h-screen  ">
          <div className="flex flex-col items-center justify-center mw-4:w-[340px] mw-9:w-[360px] mw-5:w-[400px] w-[600px]">
            ...
            <p className="py-[20px] text-[16px] mw-7:text-[14px] text-center font-inter text-white font-normal">
              Token Account Closer is a tool to close unused token accounts in
              your wallet. By closing these accounts, you will be refunded the
              SOL used to create them, which is 0.00204 SOL per account.
            </p>
            <button
              onClick={() => {
    ...
    >
      <div className="flex items-center justify-center min-h-screen ">
        <div className="flex flex-col items-center justify-center mw-4:w-[340px] mw-9:w-[360px] mw-5:w-[400px] w-[600px]">
          ...
          <p className="py-[20px] text-[16px] mw-7:text-[14px] text-center font-inter text-white font-normal">
            Token Account Closer is a tool to close unused token accounts in
            your wallet. By closing these accounts, you will be refunded the SOL
          ...
          <div className="w-full max-w-md mx-auto p-4">
            ...
            <div className="mt-4">
              ...
              {fetchingAccounts ? (
                <div className="text-center font-inter p-4">
                  Loading accounts...
                </div>
